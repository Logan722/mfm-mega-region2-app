...
import { branches, departments } from '../data/mockData';
            ...
            {Object.entries(stateGroups).map(([state, stateBranches]) => (
              <View key={state}>
                <Text style={styles.stateHeader}>
                  {state === 'TX' ? 'TEXAS' : 'FLORIDA'}
                  <Text style={styles.stateCount}> ({stateBranches.length})</Text>
                </Text>
                {stateBranches.map((branch) => (
                  <View key={branch.id} style={styles.branchCard}>
                    ...
                    <TouchableOpacity
                      ...
                      onPress={() => handleMap(branch.address)}
                    >
            ...
            {departments.map((dept) => (
              <View key={dept.id} style={styles.deptCard}>
                <View style={styles.deptIcon}>
                  <Ionicons name={dept.icon} size={24} color={Colors.purple} />
                </View>
...
const styles = StyleSheet.create({
  ...
  searchBox: {
    ...
    borderRadius: Radius.md, paddingHorizontal: Spacing.md, height: 42,
  ...
  tabBar: {
    ...
    paddingHorizontal: Spacing.base, paddingBottom: Spacing.sm, gap: 8,
  ...
  stateHeader: {
    ...
    letterSpacing: 1.5, marginTop: Spacing.base, marginBottom: Spacing.sm,
  },
  stateCount: { fontWeight: '600', color: Colors.gray400, letterSpacing: 0 },
  ...
  branchDot: {
    ...
    backgroundColor: Colors.purple, marginRight: 10,
  },
  branchName: { fontSize: 16, fontWeight: '700', color: Colors.gray800, flex: 1 },
  hqBadge: {
    backgroundColor: Colors.fire + '18', paddingHorizontal: 8, paddingVertical: 2,
