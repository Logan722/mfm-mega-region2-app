...
import { weeklyServices, specialEvents, announcements, devotionals } from '../data/mockData';

// MFM Logo from assets
const MFM_LOGO = require('../../assets/mfm-logo.png');

export default function HomeScreen({ navigation }) {
  const todayDevotional = devotionals[0];
  const todayDay = new Date().getDay();
  const todayService = weeklyServices.find(s => s.dayNum === todayDay);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <TouchableOpacity style={styles.propheticBanner} activeOpacity={0.85}>
          ...
          <View style={styles.propheticContent}>
            <View style={styles.propheticBadge}>
              ...
              <Text style={styles.propheticBadgeText}>2026 DECLARATION</Text>
            </View>
            <Text style={styles.propheticTitle}>
              "My Year of Great Deliverance{'\n'}and Fresh Glory"
            </Text>
            <Text style={styles.propheticVerse}>— Genesis 45:7</Text>
          </View>
        </TouchableOpacity>
        ...
        {/* Quick Actions — Now includes Livestream */}
        ...
        <View style={styles.section}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            {specialEvents.slice(0, 4).map((event) => (
